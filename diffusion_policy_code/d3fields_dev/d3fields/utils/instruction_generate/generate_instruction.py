...
class instruction_generater:
    ...
    def fill_in_template(self, element):
        ...
        if ("other_mug" in unique_placeholders) or ("mug" not in unique_placeholders):
            reference = element["mug"]
        else:
            reference = "mug"
        for key in element.keys():
            element[key] = element[key].format(reference = reference)
    ...
    def get_relative_position(self, layout, reference, tgt_branch_idx):
        ...
        if dist_max_idx == tgt_branch_idx:
            return ["furthest branch relative to the {reference}", "nearest branch relative to the {reference}"]
        else:
            return None
