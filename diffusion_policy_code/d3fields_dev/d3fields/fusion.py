...
from d3fields.utils.draw_utils import aggr_point_cloud_from_data, draw_keypoints
from d3fields.utils.grounded_sam import (
    ground_truth_segment,
    grounded_instance_sam_new_ver,
    grounded_instance_given_bbox,
)
from d3fields.utils.my_utils import bcolors, depth2fgpcd, fps_np
...
# NOTE: project 3D points to 2D image plane of multiple cameras
# NOTE: Rt is the extrnsic matrix of the camera, K is the intrinsic matrix of the camera
...
# NOTE: sample and interpolate the features at the points location from the feature map
...
# NOTE: resulting tensor is a 2D tensor where each row represents a point in 3D space
...
def instance2onehot(instance, N=None):
    ...
    if type(instance) is np.ndarray:
        ...
        out = torch.zeros(
            instance.shape + (N,), dtype=torch.bool, device=instance.device
        )
        for i in range(N):
            out[..., i] = instance == i
    return out
...
def rm_mask_close_to_pcd(depth, mask, pcd, K, pose):
    ...
    pcd_in_cam = depth2fgpcd(
        depth=depth, mask=mask, cam_params=cam_params, preserve_zero=True
    )
    pcd_in_world = (
        np.linalg.inv(pose)
        @ np.concatenate([pcd_in_cam, np.ones([pcd_in_cam.shape[0], 1])], axis=-1).T
    )  # [4, N]
    pcd_in_world = pcd_in_world[:3].T  # [N, 3]
    close_mask = (
        np.linalg.norm(pcd_in_world[:, None, :] - pcd[None, ...], axis=-1).min(axis=-1)
        < 0.02
    )
...
class Fusion:
    def __init__(
        ...
        #     curr_path, "ckpts/groundingdino_swint_ogc.pth"
        # )
        config_file = os.path.join(
            groundingdino.__path__[0], "config/GroundingDINO_SwinB_cfg.py"
        )
        grounded_checkpoint = os.path.join(
            curr_path, "ckpts/groundingdino_swinb_cogcoor.pth"
        )
        ...
        self.ground_dino_model = GroundingDINOModel(
            config_file, grounded_checkpoint, device=self.device
        )

        model_type = "vit_h"
        # sam_checkpoint = "/home/yitong/diffusion/diffusion_policy_code/d3fields_dev/d3fields/ckpts/mobile_sam.pt"
        mobile_sam = sam_model_registry[model_type](checkpoint=sam_checkpoint)
        mobile_sam.to(device=self.device)
        mobile_sam.eval()

        self.sam_model = SamPredictor(mobile_sam)
        ...
        self.xmem_processors = [
            InferenceCore(network, config=xmem_config) for _ in range(self.num_cam)
        ]
    ...
    def eval(self, pts, return_names=["dino_feats", "mask"], return_inter=False):
        ...
        for k in return_names:
            ...
            ).sum(0) / (dist_valid.float().sum(0).unsqueeze(-1) + 1e-6)  # [pn,k_dim]
    ...
    def eval_dist_to_sel_feats(self, feats, obj_name):
        ...
        heatmap = F.cosine_similarity(
            feats[:, None, :], sel_feats[None, ...], dim=-1
        )  # [pn, sn]
    ...
    def extract_dinov2_features(self, imgs, params):
        ...
        imgs_tensor = torch.zeros(
            (K, 3, patch_h * 14, patch_w * 14), device=self.device
        )
        for j in range(K):
            img = Image.fromarray(imgs[j])
            imgs_tensor[j] = transform(img)[:3]
        with torch.no_grad():
            features_dict = self.dinov2_feat_extractor.forward_features(
                imgs_tensor.to(dtype=self.dtype)
            )
    ...
    def xmem_process(self, rgb, mask):
        ...
        with torch.no_grad():
            ...
            for i in range(self.num_cam):
                rgb_tensor.append(
                    self.xmem_im_transform(rgb[i]).to(self.device, dtype=torch.float32)
                )
            rgb_tensor = torch.stack(rgb_tensor, dim=0)
            if self.xmem_mask_transform is not None and mask is not None:
                mask = self.xmem_mask_transform(mask).to(
            ...
            if mask is not None and not self.xmem_first_mask_loaded:
                # converted_masks = []
                for i in range(self.num_cam):
                    _, labels = self.xmem_mapper.convert_mask(
                        mask[i].cpu().numpy(), exhaustive=True
                    )
                    # converted_masks.append(converted_mask)
                converted_masks = [
                    self.xmem_mapper.convert_mask(
                        mask[i].cpu().numpy(), exhaustive=True
                    )[0]
                    for i in range(self.num_cam)
                ]
                ...
                converted_masks = torch.from_numpy(
                    np.stack(converted_masks, axis=0)
                ).to(self.device, dtype=torch.float32)
                for processor in self.xmem_processors:
                    processor.set_all_labels(list(self.xmem_mapper.remappings.values()))
                self.track_ids = [
                    0,
                ] + list(self.xmem_mapper.remappings.values())
            elif mask is not None and self.xmem_first_mask_loaded:
                converted_masks = instance2onehot(
                    mask.to(torch.uint8), len(self.track_ids)
                )
                converted_masks = converted_masks.permute(0, 3, 1, 2).to(
                    self.device, dtype=torch.float32
                )
            ...
            out_masks = torch.zeros((self.num_cam, self.H, self.W)).to(
            ...
            for view_i, processor in enumerate(self.xmem_processors):
                ...
                out_masks[view_i] = torch.from_numpy(out_mask).to(
                    self.device, dtype=torch.uint8
                )
    ...
    def update(self, obs, update_dino=True):
        ...
        self.curr_obs_torch["color_tensor"] = (
            torch.from_numpy(color).to(self.device, dtype=self.dtype) / 255.0
        )
        self.curr_obs_torch["depth"] = torch.from_numpy(obs["depth"]).to(
            self.device, dtype=self.dtype
        )
        self.curr_obs_torch["pose"] = torch.from_numpy(obs["pose"]).to(
            self.device, dtype=self.dtype
        )
        self.curr_obs_torch["K"] = torch.from_numpy(obs["K"]).to(
            self.device, dtype=self.dtype
        )
    ...
    def update_dino(self):
        color = self.curr_obs_torch["masked_color"]
        params = {
            "patch_h": color.shape[1] // 10,
            "patch_w": color.shape[2] // 10,
        }
    ...
    def pcd_iou(self, pcd_1, pcd_2, threshold):
        ...
        iou = (
            (min_dist_from_1_to_2 < threshold).sum()
            + (min_dist_from_2_to_1 < threshold).sum()
        ) / (pcd_1.shape[0] + pcd_2.shape[0])
    ...
    def merge_instances_from_new_view(self, instances_info, i, boundaries):
        ...
        for j, label in enumerate(mask_label):
            # if j == 0:
            #     continue
            pcd_i = self.extract_masked_pcd_in_views(
                [j], [i], boundaries, downsample=True
            )  # (N,3) in numpy array
            is_new_inst = True
            max_iou = 0
            max_iou_idx = -1
            for k, info in enumerate(instances_info):
                ...
                # Check 2: compute iou
                pcd_inst = np.concatenate(
                    [info["pcd"][view_idx] for view_idx in info["pcd"].keys()], axis=0
                )
                iou, _, _, _, _, _, _ = self.pcd_iou(
                    pcd_i, pcd_inst, threshold=self.iou_threshold
                )
                if iou > max_iou:
                    max_iou = iou
                    max_iou_idx = k
            ...
            if is_new_inst and (label != "background" or i == 0):
                ...
                if i in instances_info[max_iou_idx]["pcd"].keys():
                    ...
                    other_pcd_ls = [
                        instances_info[max_iou_idx]["pcd"][view_idx]
                        for view_idx in instances_info[max_iou_idx]["pcd"].keys()
                        if view_idx != i
                    ]
                    if len(other_pcd_ls) > 0:
                        other_pcd = np.concatenate(
                            [
                                instances_info[max_iou_idx]["pcd"][view_idx]
                                for view_idx in instances_info[max_iou_idx][
                                    "pcd"
                                ].keys()
                                if view_idx != i
                            ],
                        ...
                        curr_iou = self.pcd_iou(
                            pcd_i, other_pcd, threshold=self.iou_threshold
                        )[0]
                        prev_pcd = instances_info[max_iou_idx]["pcd"][i]
                        prev_iou = self.pcd_iou(
                            pcd_i, prev_pcd, threshold=self.iou_threshold
                        )[0]
    ...
    def vox_idx_iou(self, vox_idx_1, vox_idx_2):
        intersection = len(set(vox_idx_1).intersection(set(vox_idx_2)))
        union = len(set(vox_idx_1).union(set(vox_idx_2)))
        return intersection / union, len(vox_idx_1) / union, len(vox_idx_2) / union

    def merge_instances_from_new_view_vox_ver(
        self, instances_info, i, boundaries, merge_iou=0.25
    ):
        mask_label = self.curr_obs_torch["mask_label"][i]
        # assert mask_label[0] == "background"
        for j, label in enumerate(mask_label):
            ...
            if is_new_inst and (label != "background" or i == 0):
                ...
                conf = self.curr_obs_torch["mask_conf"][i][j]
                instances_info[max_iou_idx]["vox_idx"] = np.unique(
                    np.concatenate([instances_info[max_iou_idx]["vox_idx"], index_i])
                )
                if i in instances_info[max_iou_idx]["idx"]:
                    new_vox_idx = set(index_i).difference(
                        set(instances_info[max_iou_idx]["vox_idx"])
                    )
    ...
    def del_partial_pcd(self, instance_info, pcd_idx):
        ...
        for view_idx in instance_info["pcd"].keys():
            ...
            pcd_idx_in_view = pcd_idx[
                np.logical_and(pcd_idx >= start_idx, pcd_idx < end_idx)
            ]
            pcd_idx_in_view -= start_idx
            instance_info["pcd"][view_idx] = np.delete(
                instance_info["pcd"][view_idx], pcd_idx_in_view, axis=0
            )
            start_idx = end_idx
    ...
    def filter_instances(self, instances_info):
        ...
        for idx_i, instance_i in enumerate(instances_info):
            ...
            for idx_j, instance_j in enumerate(instances_info):
                ...
                pcd_i = np.concatenate(
                    [
                        instance_i["pcd"][view_idx]
                        for view_idx in instance_i["pcd"].keys()
                    ],
                    axis=0,
                )
                conf_per_pt_i = np.concatenate(
                    [
                        np.ones(instance_i["pcd"][view_idx].shape[0])
                        * instance_i["conf"][view_idx]
                        for view_idx in instance_i["pcd"].keys()
                    ]
                )
                pcd_j = np.concatenate(
                    [
                        instance_j["pcd"][view_idx]
                        for view_idx in instance_j["pcd"].keys()
                    ],
                    axis=0,
                )
                conf_per_pt_j = np.concatenate(
                    [
                        np.ones(instance_j["pcd"][view_idx].shape[0])
                        * instance_j["conf"][view_idx]
                        for view_idx in instance_j["pcd"].keys()
                    ]
                )
                (
                    ...
                    min_idx_from_1_to_2,
                    min_idx_from_2_to_1,
                ) = self.pcd_iou(pcd_i, pcd_j, threshold=0.005)
                if iou > 0.25:
                    ...
                    if num_vis_view_i > num_vis_view_j:
                        ...
                        overlap_conf_2_corr_to_1 = conf_per_pt_j[
                            min_idx_from_1_to_2[overlap_idx_1]
                        ]
                        overlap_conf_2 = conf_per_pt_j[overlap_idx_2]
                        overlap_conf_1_corr_to_2 = conf_per_pt_i[
                            min_idx_from_2_to_1[overlap_idx_2]
                        ]
                        pcd_i_del_idx = overlap_idx_1[
                            overlap_conf_1 < overlap_conf_2_corr_to_1
                        ]
                        pcd_j_del_idx = overlap_idx_2[
                            overlap_conf_2 < overlap_conf_1_corr_to_2
                        ]
                ...
                # instance_i is a subset of instance_j
                    ...
                    num_vis_view_j = len(instance_j["idx"])
                    if (
                        instance_j["label"] == "background"
                        and num_vis_view_i < self.num_cam // 2
                    ) or (
                        instance_j["label"] != "background"
                        and num_vis_view_i < num_vis_view_j // 2
                    ):
                        # delete instance_i
                        to_delete.append(idx_i)
                    else:
                ...
                # instance_j is a subset of instance_i
                    ...
                    num_vis_view_j = len(instance_j["idx"])
                    if (
                        instance_i["label"] == "background"
                        and num_vis_view_j < self.num_cam // 2
                    ) or (
                        instance_i["label"] != "background"
                        and num_vis_view_j < num_vis_view_i // 2
                    ):
                        # delete instance_j
                        to_delete.append(idx_j)
                    else:
        ...
        for idx_i, instance_i in enumerate(instances_info):
            if idx_i in to_delete:
                continue
            pcd_i = np.concatenate(
                [instance_i["pcd"][view_idx] for view_idx in instance_i["pcd"].keys()],
                axis=0,
            )
            if pcd_i.shape[0] < 10:
                to_delete.append(idx_i)
                ...
                if iou > 0.25 or iou_1 > 0.5 or iou_2 > 0.5:
                    ...
                    for vox_i in conf_per_pt_i.keys():
                        ...
                        if len(conf_per_pt_i[vox_i]) < len(conf_per_pt_j[vox_i]):
                            ...
                            if np.mean(conf_per_pt_i[vox_i]) < np.mean(
                                conf_per_pt_j[vox_i]
                            ):
                                to_delete_i.append(vox_i)
                    to_delete_j = []
                    for vox_j in conf_per_pt_j.keys():
                        ...
                        if len(conf_per_pt_j[vox_j]) < len(conf_per_pt_i[vox_j]):
                            ...
                            if np.mean(conf_per_pt_j[vox_j]) < np.mean(
                                conf_per_pt_i[vox_j]
                            ):
                                to_delete_j.append(vox_j)
    ...
    def swap_instance_mask(self, instances_info):
        self.curr_obs_torch["mask"] = torch.zeros(
            (self.num_cam, self.H, self.W), device=self.device, dtype=torch.uint8
        )
        for i in range(self.num_cam):
            new_mask = np.zeros_like(
                self.curr_obs_torch["mask_gs"][i][0], dtype=np.uint8
            )
            ...
            self.curr_obs_torch["mask"][i] = torch.from_numpy(new_mask).to(
                device=self.device, dtype=torch.uint8
            )

    def align_instance_mask_v3(
        self, queries, boundaries, expected_labels=None, voxel_size=0.02, merge_iou=0.25
    ):
        queries = ["battery"]
        ...
        lower_bound = np.array(
            [boundaries["x_lower"], boundaries["y_lower"], boundaries["z_lower"]]
        )
        higher_bound = np.array(
            [boundaries["x_upper"], boundaries["y_upper"], boundaries["z_upper"]]
        )
        ...
        for i in range(self.num_cam):
            instances_info = self.merge_instances_from_new_view_vox_ver(
                instances_info, i, boundaries, merge_iou=merge_iou
            )
        ...
        if instances_info[0]["label"] != "background":
            instances_info.insert(
                0,
                {
                    "label": "background",
                    "vox_idx": np.array([], dtype=np.int32),
                    "conf_per_pt": {},
                    "idx": {},
                },
            )
        ...
        self.curr_obs_torch["consensus_mask_label"] = [
            instance_info["label"] for instance_info in instances_info
        ]
        # if (
        #     expected_labels is not None
        #     and self.curr_obs_torch["consensus_mask_label"] != expected_labels
        # ):
        print(
            f"{bcolors.OKGREEN}consensus mask label: ",
            self.curr_obs_torch["consensus_mask_label"],
            f"{bcolors.ENDC}",
        )
    ...
    def text_queries_for_inst_mask_no_track(
        ...
        bboxs = []
        for i in range(self.num_cam):
            img = self.curr_obs_torch["color"][i]
            img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
            mask, label, mask_conf, bbox = grounded_instance_sam_new_ver(
                img,
                queries,
                self.ground_dino_model,
                self.sam_model,
                thresholds,
                merge_all,
                device=self.device,
            ...
            if robot_pcd is not None:
                ...
                for inst_i in range(mask.shape[0]):
                    pose_i = (
                        self.curr_obs_torch["pose"][i].detach().cpu().numpy()
                    )  # [3,4]
                    pose_i = np.concatenate(
                        [pose_i, np.array([[0, 0, 0, 1]])], axis=0
                    )  # [4,4]
                    mask[inst_i] = rm_mask_close_to_pcd(
                        depth=self.curr_obs_torch["depth"][i].detach().cpu().numpy(),
                        mask=mask[inst_i],
                        pcd=robot_pcd,
        ...
        self.curr_obs_torch["bbox"] = bboxs  # [num_cam, num_obj, 4]
        _, idx = np.unique(labels[0], return_index=True)
        self.curr_obs_torch["semantic_label"] = list(
            np.array(labels[0])[np.sort(idx)]
        )  # list of semantic label we have
        ...
        if self.vis_mask:
            if self.use_plotly:
                for i in range(self.num_cam):
                    image = self.curr_obs_torch["color"][i]
                    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                    label = self.curr_obs_torch["mask_label"][i][1:]
                    score = self.curr_obs_torch["mask_conf"][i][1:]
                    box = self.curr_obs_torch["bbox"][i]
                    mask = self.curr_obs_torch["mask_gs"][i][1:]
                    annotated_img = self.annotate_plotly(
                        image, label, score, box, mask, get_class_color_dict()
                    )
            else:
                plt.figure()
                for i in range(self.num_cam):
                    ...
                    box = self.curr_obs_torch["bbox"][i]
                    mask = self.curr_obs_torch["mask_gs"][i][1:]
                    annotated_img = self.annotate_cv2(image, label, score, box, mask)
                    plt.subplot(1, self.num_cam, i + 1)
                    plt.imshow(annotated_img)
                    plt.axis("off")
                plt.show()

        self.align_instance_mask_v3(
            queries,
            boundaries,
            expected_labels,
        ...
        self.curr_obs_torch["mask"] = instance2onehot(
            self.curr_obs_torch["mask"].to(torch.uint8),
            len(self.curr_obs_torch["consensus_mask_label"]),
        ).to(dtype=self.dtype)


    def bbox_for_inst_mask(
        self,
        bbox_list,
        boundaries,
        use_sam=False,
        ...
        if not self.xmem_first_mask_loaded:
            ...
            for i in range(self.num_cam):
                mask, label, mask_conf, bbox = grounded_instance_given_bbox(
                    bbox_list[i],
                    self.curr_obs_torch["color"][i],
                    self.sam_model,
                )
                ...
                mask_confs.append(mask_conf)
                bboxs.append(bbox)
            self.curr_obs_torch["mask_gs"] = masks  # [num_cam, num_obj, H, W]
            self.curr_obs_torch["mask_label"] = labels  # [num_cam, ] list of list
            self.curr_obs_torch["mask_conf"] = mask_confs  # [num_cam, ] list of list
            self.curr_obs_torch["bbox"] = bboxs  # [num_cam, num_obj, 4]
            _, idx = np.unique(labels[0], return_index=True)
            ...
            if self.vis_sam or self.save_vis_sam:
                for i in range(self.num_cam):
                    image = self.curr_obs_torch["color"][i]
                    label = self.curr_obs_torch["mask_label"][i][1:]
                    score = self.curr_obs_torch["mask_conf"][i][1:]
                    box = self.curr_obs_torch["bbox"][i]
                    mask = self.curr_obs_torch["mask_gs"][i][1:]
                    self.annotate_plotly(
                        image, label, score, box, mask, get_class_color_dict(), i
                    )
            # self.align_instance_mask_v3(
            #     ["inst_0", "inst_1", "inst_2", "inst_3", "inst_4", "inst_5", "inst_6"],
            #     boundaries,
            #     expected_labels,
            #     voxel_size=voxel_size,
            #     merge_iou=merge_iou,
            # )
            self.curr_obs_torch["consensus_mask_label"] = labels[0]
            self.curr_obs_torch["mask"] = (
                torch.from_numpy(np.stack(masks)).to(self.device).permute(0, 2, 3, 1)
            )
            self.curr_obs_torch["mask"] = onehot2instance(
                self.curr_obs_torch["mask"].to(torch.uint8)
            )
            if track:
                self.curr_obs_torch["mask"] = self.xmem_process(
                    self.curr_obs_torch["color"], self.curr_obs_torch["mask"]
                ).to(dtype=self.dtype)
            else:
                self.curr_obs_torch["mask"] = instance2onehot(
                    self.curr_obs_torch["mask"].to(torch.uint8),
            ...
            self.curr_obs_torch["mask"] = self.xmem_process(
                self.curr_obs_torch["color"], None
            )  # [num_cam, H, W, num_instance]
            self.curr_obs_torch["mask"] = self.curr_obs_torch["mask"].to(
                dtype=self.dtype
            )
        else:
            raise NotImplementedError
        ...
        if False:
            if self.vis_xmem or self.save_vis_xmem:
                annotated_img = []
                for i in range(self.num_cam):
                    image = self.curr_obs_torch["color"][i]
                    # image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                    label = self.curr_obs_torch["consensus_mask_label"][1:]
                    mask = (
                        self.curr_obs_torch["mask"][i].detach().cpu().numpy().astype(bool)
                    )
                    mask = np.moveaxis(mask, -1, 0)[1:]
                    annotated_img.append(
                        self.annotate_cv2(image, mask, label, get_class_color_dict())
                    )
                annotated_img = np.concatenate(annotated_img, axis=1)

                if self.vis_xmem:
                    cv2.imshow("XMem", annotated_img)
                ...
                if self.save_vis_xmem:
                    assert hasattr(self, "video_recorder")
                    assert type(self.save_dir) is str
                    save_path = os.path.join(
                        self.save_dir, "visualization", f"{self.save_idx}"
                    )
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    if not self.video_recorder.is_ready():
                        self.video_recorder.start(os.path.join(save_path, "vis_xmem.mp4"))
                    self.video_recorder.write_frame(annotated_img)

    def text_queries_for_inst_mask(
        self,
        queries,
        env_position,
        thresholds,
        ...
        if not self.xmem_first_mask_loaded:
            ...
            bboxs = []
            for i in range(self.num_cam):
                # mask_1, label_1, mask_conf_1, bbox_1 = grounded_instance_sam_new_ver(
                #     self.curr_obs_torch["color"][i],
                #     queries,
                #     # exclude_queries,
                #     self.ground_dino_model,
                ...
                #     device=self.device,
                # )
                mask_2, label_2, mask_conf_2, bbox_2 = ground_truth_segment(
                    image=self.curr_obs_torch["color"][i],
                    env_config=env_position,
                    obs=self.curr_obs_torch,
                    cam_idx=i,
                    sam_model=self.sam_model,
                )
                # mask = np.concatenate([mask_1, mask_2],axis=0)
                # label = label_1 + label_2
                # mask_conf = np.concatenate([mask_conf_1, mask_conf_2],axis=0)
                # bbox = np.concatenate([bbox_1, bbox_2],axis=0)
                mask = mask_2
                label = label_2
                mask_conf = mask_conf_2
                bbox = bbox_2
                ...
                if robot_pcd is not None:
                    ...
                    for inst_i in range(mask.shape[0]):
                        ...
                        mask[inst_i] = rm_mask_close_to_pcd(
                            depth=self.curr_obs_torch["depth"][i]
                            .detach()
                            .cpu()
                            .numpy(),
                            mask=mask[inst_i],
            ...
            if self.vis_mask:
                if self.use_plotly:
                    ...
                    for i in range(self.num_cam):
                        ...
                        box = self.curr_obs_torch["bbox"][i]
                        mask = self.curr_obs_torch["mask_gs"][i][1:]
                        annotated_img = self.annotate_cv2(
                            image, label, score, box, mask
                        )
                        plt.subplot(1, self.num_cam, i + 1)
                        plt.imshow(annotated_img)
                        plt.axis("off")
    ...
    def crop_image(self, label, t):
        masked_img_list = list()
        for i in range(self.num_cam):
            rgb_img = self.curr_obs_torch["color"][i]
            mask_gs_full = self.curr_obs_torch["mask"][i].cpu().numpy()
            mask_label_full = self.curr_obs_torch["mask_label"][i]
            sel_idx = mask_label_full.index(label)
            mask_gs_sel = mask_gs_full[..., sel_idx]
            masked_img = rgb_img * mask_gs_sel[:, :, None]
            masked_img_list.append(masked_img[None, ...])
        masked_img_list = np.concatenate(masked_img_list, axis=0)
        self.curr_obs_torch["masked_color"] = masked_img_list.astype(np.uint8)
        np.save(f"/root/lyt/test_mask/test_masked_img_{t}.npy", masked_img_list)

    def annotate_cv2(self, image, label, score, box, mask):
        ...
        for i in range(num_obj):
            ...
            cv2.rectangle(image, (xmin, ymin), (xmax, ymax), color.tolist(), 2)
            cv2.putText(
                image,
                f"{label[i]}: {score[i]:.2f}",
                (xmin, ymin - 10),
            ...
            if mask is not None:
                ...
                mask_uint8 = (mask[i] * 255).astype(np.uint8)
    ...
    def annotate_plotly(self, image, label, score, box, mask, class_color_dict=None):
        ...
        button_shapes = button_shapes + [
            dict(
                label=f"Detection {idx + 1}", method="relayout", args=["shapes", shape]
            )
    ...
    def get_mask_idx_dict(self):
        # NOTE: excluding the background
        mask_idx_list = list(range(len(self.curr_obs_torch["consensus_mask_label"])))
        mask_idx_dict = {}
        for idx, item in enumerate(self.curr_obs_torch["consensus_mask_label"]):
            if item not in mask_idx_dict:
                mask_idx_dict[item] = [idx]
            else:
                mask_idx_dict[item].append(idx)
        if "background" in mask_idx_dict:
            for idx in mask_idx_dict["background"]:
                mask_idx_list.pop(idx)
            mask_idx_dict.pop("background")
        return mask_idx_list, mask_idx_dict

    def get_mask(self, label):
        mask_label = self.curr_obs_torch["mask_label"]
        mask = self.curr_obs_torch["mask"].cpu().numpy()
        mask_list = []
        for cam_idx in range(len(mask_label)):
            
            mask_label_per_view = mask_label[cam_idx]
            inst_idx = mask_label_per_view.index(label)
            mask_list.append(mask[cam_idx,...,inst_idx][None, ..., None])
        
        tgt_mask = np.concatenate(mask_list, axis=0)
        return tgt_mask

    def project_pcd_to_mask(self, pcd, pose, K):
        ...
        for i in range(num_views):
            ...
            pcd_camera = np.dot(R, pcd_homogeneous[:, :-1].T) + t[:, np.newaxis]  
            p_pixel_hom = np.dot(K[i], pcd_camera)
            
            p_pixel = p_pixel_hom[:2] / p_pixel_hom[2] 
            u = np.round(p_pixel[0]).astype(int)
            v = np.round(p_pixel[1]).astype(int)
            ...
            mask = self.dilate_mask(mask, 4)

            masks.append(mask)

        return np.array(masks)[...,None]

    def dilate_mask(self,mask, threshold_distance):

        from scipy.ndimage import distance_transform_edt
        distance = distance_transform_edt(mask == 0) 
        dilated_mask = (distance <= threshold_distance).astype(np.uint8)
    ...
    def extract_masked_pcd(self, inst_idx_ls, boundaries=None):
        ...
        for i in range(self.num_cam):
            sel_mask[i] = (
                cv2.erode(
                    (sel_mask[i] * 255).astype(np.uint8),
        ...
        pcd, _ = aggr_point_cloud_from_data(
            color,
            depth,
            K,
            pose,
    ...
    def extract_masked_pcd_in_views(
        self, inst_idx_ls, view_idx_ls, boundaries, downsample=True
    ):
        ...
        mask = np.stack(
            [self.curr_obs_torch["mask_gs"][view_idx] for view_idx in view_idx_ls],
            axis=0,
        ).transpose(0, 2, 3, 1)  # [num_view, H, W, num_inst]
        ...
        for view_idx in range(sel_mask.shape[0]):
            sel_mask[view_idx] = (
                cv2.erode(
                    (sel_mask[view_idx] * 255).astype(np.uint8),
                    np.ones([2, 2], np.uint8),
                    iterations=1,
                )
                / 255
    ...
    def get_query_obj_pcd(self):
        ...
        pcd = aggr_point_cloud_from_data(
            color, depth, K, pose, downsample=False, masks=mask
        )
        return pcd
    ...
    def create_instance_mask_mesh(self, vertices, triangles, res, out_o3d=False):
        """
        Create the instance mask mesh for visualization
        ...
        for k in res.keys():
            if k.startswith("mask"):
                ...
                color_palette = sns.color_palette("deep") + sns.color_palette("bright")
                color_palette = np.asarray(color_palette)
                mask = onehot2instance(mask)  # (N, nq) -> (N)
                vertices_color = np.zeros((vertices.shape[0], 3))
                for i in range(num_instance):
                    vertices_color[mask == i] = color_palette[i]
                if out_o3d:
                    o3d_mesh = o3d.geometry.TriangleMesh(
                        vertices=o3d.utility.Vector3dVector(vertices),
                        triangles=o3d.utility.Vector3iVector(triangles[..., ::-1]),
                    )
                    o3d_mesh.vertex_colors = o3d.utility.Vector3dVector(vertices_color)
                else:
                    vertices_color = (vertices_color * 255).astype(np.uint8)
                    vertices_color = np.concatenate(
                        [
                            vertices_color,
                            np.ones((vertices_color.shape[0], 1), dtype=np.uint8) * 255,
                        ],
                        axis=1,
                    )
                ...
                mask_meshes.append(o3d_mesh)
        return mask_meshes

    def create_descriptor_mesh(
        self, vertices, triangles, res, params, mask_out_bg, out_o3d=False, z_lower=0.01
    ):
        pca = params["pca"]
        ...
        for i in range(features_pca.shape[1]):
            if not mask_out_bg:
                features_rgb[:, i] = (features_pca[:, i] - features_pca[:, i].min()) / (
                    features_pca[:, i].max() - features_pca[:, i].min()
                )
            else:
                features_rgb[:, i] = (features_pca[:, i] - features_pca[:, i].min()) / (
                    features_pca[:, i].max() - features_pca[:, i].min()
        ...
        if out_o3d:
            features_mesh = o3d.geometry.TriangleMesh(
                vertices=o3d.utility.Vector3dVector(vertices),
                triangles=o3d.utility.Vector3iVector(triangles[..., ::-1]),
            )
            features_mesh.vertex_colors = o3d.utility.Vector3dVector(features_rgb)
        else:
            features_rgb = (features_rgb * 255).astype(np.uint8)
            features_rgb = np.concatenate(
                [
                    features_rgb,
                    np.ones((features_rgb.shape[0], 1), dtype=np.uint8) * 255,
                ],
                axis=1,
            )
            features_mesh = trimesh.Trimesh(
                vertices=vertices,
                faces=triangles[..., ::-1],
                vertex_colors=features_rgb,
            )
        return features_mesh, features_pca.max(axis=0), features_pca.min(axis=0)

    def create_color_mesh(self, vertices, triangles, res, out_o3d=False):
        colors = res["color_tensor"].detach().cpu().numpy()
        if out_o3d:
            color_mesh = o3d.geometry.TriangleMesh(
                vertices=o3d.utility.Vector3dVector(vertices),
                triangles=o3d.utility.Vector3iVector(triangles[..., ::-1]),
            )
            color_mesh.vertex_colors = o3d.utility.Vector3dVector(colors[:, :3])
        else:
            colors = (colors * 255).astype(np.uint8)
            colors = np.concatenate(
                [colors, np.ones((colors.shape[0], 1), dtype=np.uint8) * 255], axis=1
            )
            color_mesh = trimesh.Trimesh(
                vertices=vertices, faces=triangles[..., ::-1], vertex_colors=colors
            )
        return color_mesh

    def select_features_rand(
        self, boundaries, N, per_instance=False, res=None, init_idx=-1
    ):
        ...
        for i in range(1, len(label)):
            ...
            sample_pts, sample_idx, _ = fps_np(
                masked_pts.detach().cpu().numpy(), N, init_idx=init_idx
            )
            # src_feats_list.append(out['dino_feats'][sample_idx])
            src_feats_list.append(
                self.eval(torch.from_numpy(sample_pts).to(self.device, torch.float32))[
                    "dino_feats"
                ]
            )
            ...
            sample_pts = np.concatenate(
                [sample_pts, np.ones([num_pts, 1])], axis=-1
            )  # [num_pts, 4]
    ...
    def select_features_from_pcd(
        ...
        if not use_seg:
            ...
            if N > 0:
                sample_pts, index = sample_farthest_points(
                    masked_pts[None], K=N, random_start_point=False
                )
                sample_pts = sample_pts[0].detach().cpu().numpy()
                if color is not None:
                    color_tensor = torch.from_numpy(color).to(self.device).to(self.dtype)
                    color_tensor = color_tensor[dist_mask & out["valid_mask"]]
                    color_tensor = color_tensor[index,:] 
                    color = color_tensor.detach().cpu().numpy()
            ...
            if use_dino:
                src_feats_list.append(
                    self.eval(
                        torch.from_numpy(sample_pts).to(self.device, self.dtype),
                        return_names=["dino_feats"],
                    )["dino_feats"]
                )
            return src_feats_list, src_pts_list, img_list, color_list

        label = self.curr_obs_torch["consensus_mask_label"]

        back_ground_label = label[0]
        ...
        label_list = list()
        for i in range(1, len(label)):
            if label[i] == back_ground_label and not per_instance:
                continue
            instance_mask = mask[:, i] > 0.6
            masked_pts = pcd_tensor[instance_mask & dist_mask & out["valid_mask"]]

            if masked_pts.shape[0] <= N / 5:
                continue

            if masked_pts.shape[0] >= N:
                sample_pts, index = sample_farthest_points(
                    masked_pts[None], K=N, random_start_point=False
                )
            else:
                print(label[i], masked_pts.shape)
                sample_pts, index = sample_farthest_points(
                    masked_pts[None],
                    K=masked_pts.shape[-1] - 1,
            ...
            if use_dino:
                src_feats_list.append(
                    self.eval(torch.from_numpy(sample_pts).to(self.device, self.dtype))[
                        "dino_feats"
                    ]
                )
            src_pts_list.append(sample_pts)
        ...
        del out
        return src_feats_list, src_pts_list, img_list, label_list
    ...
    def compute_attn(
        ...
        """compute the attention map of the pcd

        Args:
            pcd (np.ndarray): the point cloud - assuming pcd are valid
            mask_idx (List[int]): the indices of mask that we are interested in
            fixed_pts (np.ndarray): the fixed points. If pcd is close to fixed_pts, the attention map should be high.
        ...
        # fixed pts
        if len(fixed_pts) > 0:
            dist = np.linalg.norm(pcd[None] - fixed_pts[:, None, :3], axis=2).min(
                axis=0
            )
    ...
    def select_features_rand_v2(self, boundaries, N, per_instance=False):
        ...
        for i in range(1, len(label)):
            ...
            for cam_i in range(self.num_cam):
                instance_mask = (
                    (self.curr_obs_torch["mask"][cam_i, :, :, i])
                    .detach()
                    .cpu()
                    .numpy()
                    .astype(bool)
                ...
                instance_mask = cv2.erode(
                    instance_mask, np.ones([15, 15], np.uint8), iterations=1
                )
                # plt.subplot(1, 2, 2)
                # plt.imshow(instance_mask)
                # plt.show()
                ...
                src_pts = np.matmul(
                    np.linalg.inv(pose_i),
                    np.concatenate([src_pts, np.ones([N_per_cam, 1])], axis=-1).T,
                )[:3].T  # [num_pts, 3] # camera to world
    ...
    def rigid_tracking(
        self, src_feat_info, last_match_pts_list, boundaries, rand_ptcl_num
    ):
        lr = 0.01
        iter_num = 100
        reg_w = 1.0
        ...
        src_feats = torch.cat(
            src_feats, dim=0
        )  # [num_instance * rand_ptcl_num, feat_dim]

        num_instance = len(last_match_pts_list)
        last_match_pts_np = np.stack(
            last_match_pts_list, axis=0
        )  # [num_instance, rand_ptcl_num, 3]
        assert last_match_pts_np.shape[:2] == (num_instance, rand_ptcl_num)

        last_match_pts_tensor = torch.from_numpy(last_match_pts_np).to(
            self.device, dtype=torch.float32
        )
        ...
        optimizer = torch.optim.Adam(
            [t_params, log_r_params], lr=lr, betas=(0.9, 0.999)
        )
        ...
        for iter_idx in range(iter_num):
            ...
            T = (
                Transform3d(device=self.device, dtype=torch.float32)
                .rotate(r_params)
                .translate(t_params)
            )
            ...
            curr_dist = (
                out["dist"] * out["valid_mask"]
            )  # [num_instance * rand_ptcl_num]
            feat_loss = (
                torch.norm((curr_feats - src_feats), dim=-1) * out["valid_mask"]
            ).mean()
            ...
            oob_loss = (
                oob_w
                * (
                    torch.clamp(curr_match_pts[:, 0] - boundaries["x_upper"], min=0)
                    ...
                    + torch.clamp(boundaries["z_lower"] - curr_match_pts[:, 2], min=0)
                ).mean()
            )
        ...
        match_pts_list = []
        for i in range(num_instance):
            match_pts_list.append(
                curr_match_pts[i * rand_ptcl_num : (i + 1) * rand_ptcl_num]
                .detach()
                .cpu()
                .numpy()
            )
    ...
    def vis_curr_mask(self):
        ...
        for i in range(self.num_cam):
            ...
            mask = (
                self.curr_obs_torch["mask"][i].detach().cpu().numpy()
            )  # [H, W, num_instance]
...
def compare_dino_dinov2_time():
    ...
    device = "cuda"
    fusion = Fusion(
        num_cam=4, feat_backbone=model_type, device=device, dtype=torch.float16
    )
    for i in range(11):
        if i == 1:
...
def compare_float_prec():
    ...
    fusion_full = Fusion(
        num_cam=4, feat_backbone=model_type, device=device, dtype=torch.float32
    )
...
def test_grounded_sam():
    ...
    config_file = os.path.join(
        curr_path, "GroundingDINO/groundingdino/config/GroundingDINO_SwinT_OGC.py"
    )
    ...
    if not os.path.exists(sam_checkpoint):
        ...
        os.system(f"mv sam_vit_h_4b8939.pth {ckpts_dir}")
    ground_dino_model = GroundingDINOModel(
    ...
    mask, label, mask_conf, _ = grounded_instance_sam_new_ver(
        img, queries, ground_dino_model, sam_model, thresholds
    )
    plt.imshow(mask[0])
    plt.show()
